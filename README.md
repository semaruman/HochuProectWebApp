...
Регистрация → создание/публикация проекта → отклик → принятие отклика (сделка) → fund → чат → сдача → приёмка → отзыв.

Оплата в MVP — `StubPaymentService` (`IPaymentService`), без хранения карт.
